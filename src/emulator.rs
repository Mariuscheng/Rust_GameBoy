...
const DEBUG_FRAME_INFO: bool = true; // 每 30 幀列印 VRAM/鍵盤摘要
...
pub fn run_emulator() {
    ...
    let mut display_manager =
        DisplayManager::new("Rust GB", scale, user_volume).expect("Display init failed");

    // 讓 Bus 能更新音色
    display_manager.attach_synth(&mut cpu.memory);
    ...
    while !quit {
        ...
        // Update APU for each CPU cycle
        display_manager.tick_apu();
        ...
        // 輸入更新（joypad），並檢查是否退出
        quit = display_manager.pump_events_and_update_joypad(|dpad, btns| {
            cpu.memory.set_joypad_rows(dpad, btns);
        });

        // 按鍵測試：偵測邊緣事件並印出
        if DEBUG_FRAME_INFO {
            InputManager::handle_debug_keys(&mut display_manager.display);
        }

        // 在 VBlank 上升沿執行 blit，避免撕裂與閃爍
        // VBlank 上升沿：LY 從 143 → 144（此時前一幀剛剛渲染完畢，PPU 停止寫入）
        let in_vblank = cpu.memory.read(0xFF44) >= 144;
        if in_vblank && !prev_in_vblank {
            frame_counter += 1;
            // 交換前後緩衝，取得完整的前一幀
            cpu.memory.swap_framebuffer();
            // Debug: 確認上一幀是否每一掃描線都已渲染完成
            let lines_done = cpu.memory.rendered_line_count();
            if DEBUG_FRAME_INFO && lines_done < SCREEN_H as usize {
                println!(
                    "[WARN] Frame {} incomplete: {} / {} lines rendered",
                    frame_counter - 1,
                    lines_done,
                    SCREEN_H
                );
            }
            // 直接取用顯示緩衝（back buffer）並複製到 shades_buf
            let fb = cpu.memory.framebuffer();
            shades_buf.copy_from_slice(fb);
            // 不再在 LCD 關閉時強制清白，保留上一幀內容以符合多數真機觀感
            let _ = display_manager.blit_framebuffer(&shades_buf);
            ...
            if DEBUG_FRAME_INFO && frame_counter % 30 == 0 {
                ...
                let keyboard = display_manager.display.event_pump.keyboard_state();
                let keys = [
                    ("R", Scancode::Right),
                    ("L", Scancode::Left),
                    ("U", Scancode::Up),
            ...
            // 精準節流到 ~59.73 FPS（依 VBlank 上升沿節點）
            let since = last_vblank.elapsed();
            if since < target_frame {
                let sleep_dur = target_frame - since;
                // 只在差距大於 1ms 時才睡眠,減少閃爍
        ...
        // VBlank 期間：依 CPU 週期節流，維持實時間 1x 速度
        if in_vblank {
...
pub fn try_load_rom(cpu: &mut GB::CPU::CPU, rom_arg: Option<String>) -> bool {
...
pub fn init_post_bios(cpu: &mut GB::CPU::CPU) {
