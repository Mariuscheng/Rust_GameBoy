...
fn try_load_rom(cpu: &mut GB::CPU::CPU, rom_arg: Option<String>) -> Option<String> {
    ...
    let candidates = [
        Path::new("01-registers.gb"),
        Path::new("02-len ctr.gb"),
        Path::new("03-trigger.gb"),
        Path::new("04-sweep.gb"),
        Path::new("05-sweep details.gb"),
        Path::new("06-overflow on trigger.gb"),
        Path::new("07-len sweep period sync.gb"),
        Path::new("08-len ctr during power.gb"),
        Path::new("09-wave read while on.gb"),
        Path::new("10-wave trigger while on.gb"),
        Path::new("11-regs after power.gb"),
        Path::new("12-wave write while on.gb"),
        Path::new("cpu_instrs.gb"),
        Path::new("dmg_sound.gb"),
        Path::new("dmg-acid2.gb"),
        Path::new("interrupt_time.gb"),
        Path::new("mem_timing_1.gb"),
        Path::new("mem_timing_2.gb"),
        Path::new("oam_bug.gb"),
        Path::new("tetris.gb"),
...
fn main() {
    ...
    let rom_path = try_load_rom(&mut cpu, rom_arg.clone());

    // 跳過 BIOS 的常見初始化（Pan Docs post-BIOS defaults）
    init_post_bios(&mut cpu);
    // 主動 power on APU，確保 blargg 測試能正確啟動序列輸出
    cpu.memory.write(0xFF26, 0x80);
    ...
    while !quit {
        ...
        if in_vblank && !prev_in_vblank {
            ...
            // Optional: exit after a fixed number of frames if env var is set (used by tests)
            if let Ok(frames_to_run) = std::env::var("GB_RUN_FRAMES") {
                if let Ok(n) = frames_to_run.parse::<u64>() {
                    if frame_counter >= n {
                        quit = true;
                    }
                }
