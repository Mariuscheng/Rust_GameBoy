...
fn ei_exec(_instr: &Instruction, cpu: &mut crate::GB::CPU::CPU) -> u64 {
    ...
    #[cfg(feature = "dbg_int")]
    {
        println!("[EI] delay=1 at pc=0x{:04X}", cpu.registers.get_pc());
    }
...
fn di_exec(_instr: &Instruction, cpu: &mut crate::GB::CPU::CPU) -> u64 {
    #[cfg(feature = "dbg_int")]
    {
        println!("[DI] ime=false at pc=0x{:04X}", cpu.registers.get_pc());
    }
    cpu.ime = false;
...
fn halt_exec(_instr: &Instruction, _cpu: &mut crate::GB::CPU::CPU) -> u64 {
    ...
    {
        println!(
            "[HALT] IME={} pending=0x{:02X} at pc=0x{:04X} -> {}",
            ime,
            pending,
            _cpu.registers.get_pc(),
            if !ime && pending != 0 {
                "halt_bug"
            } else {
                "halted"
