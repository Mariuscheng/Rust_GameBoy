...
use crate::GB::BUS::Bus; // Import of registers submodule // Temporary memory until MMU

pub struct CPU {
    pub registers: registers::Registers,
    pub ime: bool, // Interrupt Master Enable - True if you want to enable and intercept interrupts
    // EI delay mechanism: EI sets delay counter; after delay expires, IME becomes true.
    pub ime_enable_delay: u8, // Delay counter for EI instruction (0 = no delay, 1 = enable after next instruction)
    pub ime_just_enabled: bool, // Flag: EI delay expired this cycle, don't service interrupts yet
...
#[cfg(test)]
mod dma_timing_tests {
    use super::*;

    #[test]
    fn interrupt_serviced_while_dma_active() {
        let mut cpu = CPU::new();
        for i in 0..160u16 {
        ...
        assert!(cpu.memory.is_dma_active());
    }

    #[test]
    fn dma_progresses_during_interrupt_service() {
        let mut cpu = CPU::new();
        for i in 0..160u16 {
            cpu.memory.write(0x8000 + i, (i & 0xFF) as u8);
        }
        ...
        cpu.memory.write(0xFFFF, 0x01);
        cpu.memory.write(0xFF0F, 0x01);
        let before = cpu.memory.read(0xFE01);
        let taken = cpu.execute_next();
        assert_eq!(taken, 20);
        let after = cpu.memory.read(0xFE01);
        assert_ne!(before, after);
        let mut copied = 0;
        for i in 0..160u16 {
            if cpu.memory.read(0xFE00 + i) != 0 {
                copied += 1;
            }
        }
        assert!(copied > 0);
    ...
    #[test]
    fn dma_restart_mid_transfer_uses_new_source() {
        let mut cpu = CPU::new();
        // Prepare two sources: 0x8000 and 0x9000 with distinct patterns
        for i in 0..160u16 {
            cpu.memory.write(0x8000 + i, 0xAA);
            cpu.memory.write(0x9000 + i, 0x55);
        }

        // Start DMA from 0x8000
        cpu.memory.write(0xFF46, 0x80);
        // Let DMA run for a portion (e.g., 200 cycles)
        cpu.memory.step(200);
        assert!(cpu.memory.is_dma_active());

        // While DMA active, write FF46 to restart from 0x9000
        cpu.memory.write(0xFF46, 0x90);

        // Advance enough cycles to finish the restarted DMA
        cpu.memory.step(640);

        // Verify OAM now matches the second source (0x55)
        for i in 0..160u16 {
            let v = cpu.memory.read(0xFE00 + i);
            assert_eq!(v, 0x55);
        }
    }

    #[test]
    fn dma_oam_overlap_source_equals_dest() {
        let mut cpu = CPU::new();
        // Fill OAM with a distinct pattern
        for i in 0..160u16 {
            cpu.memory
                .write(0xFE00 + i, (i.wrapping_mul(3) & 0xFF) as u8);
        }
        // Snapshot before DMA
        let mut before = [0u8; 160];
        for i in 0..160u16 {
            before[i as usize] = cpu.memory.read(0xFE00 + i);
        }

        // Start DMA with source = 0xFE00 (write 0xFE to 0xFF46)
        cpu.memory.write(0xFF46, 0xFE);
        // Advance time to complete DMA
        cpu.memory.step(640);

        // OAM should equal the snapshot (copying from itself should preserve data)
        for i in 0..160u16 {
            assert_eq!(cpu.memory.read(0xFE00 + i), before[i as usize]);
        }
    }
}
...
impl CPU {
    ...
    fn service_interrupt(&mut self, bit: u8, vector: u16, pc_to_push: u16) -> u64 {
        // According to Pan Docs the sequence is:
        // 1) IME -> 0
        // 2) Clear IF bit
        // 3) Two byte push of PC (high then low)
        // 4) PC <- vector
        // Total time: 5 machine cycles (20 t-cycles)
        // We also tick peripherals AFTER adjusting PC so return path timing matches blargg tests.
        #[cfg(feature = "dbg_int")]
        {
            println!(
                "[INT] Servicing interrupt bit={} vec=0x{:04X} at pc=0x{:04X} IE={:02X} IF={:02X}",
                bit,
                vector,
                pc_to_push,
                self.read_ie(),
        ...
        self.ime = false;
        self.ime_just_enabled = false; // Clear the flag when servicing
        let mut ifv = self.read_if();
        ifv &= !(1u8 << bit);
        self.write_if(ifv);
        let ret = pc_to_push;
        let sp_high = self.registers.get_sp().wrapping_sub(1);
        self.write8(sp_high, (ret >> 8) as u8);
        let sp_low = sp_high.wrapping_sub(1);
        self.write8(sp_low, (ret & 0xFF) as u8);
    ...
    pub fn fetch_next(&mut self) -> u8 {
        ...
        // Store a global last-pc so bus memory writes (eg. serial) can associate PC with writes
        debug::LAST_PC.store(pc, std::sync::atomic::Ordering::SeqCst);
    ...
    pub fn execute_next(&mut self) -> u64 {
        ...
        // Handle debug for RST 38 loop detection
        self.handle_rst38_debug();

        // Apply EI delayed enable at the start of every instruction
        self.apply_ei_delay();

        // Check for pending interrupts before executing instruction
        let interrupt_cycles_before = self.check_and_service_interrupts_before();

        if interrupt_cycles_before > 0 {
            return interrupt_cycles_before;
        }

        // Execute instruction (cycles already added in execute_instruction)
        let instr_cycles = self.execute_instruction();

        // Check and service interrupts after instruction execution if EI delay allows
        let interrupt_cycles_after = self.check_and_service_interrupts_after();

        // Return total cycles: instruction + interrupt service
        instr_cycles.wrapping_add(interrupt_cycles_after)
    }

    // Handle debug detection for RST 38 loop
    fn handle_rst38_debug(&mut self) {
        if !self.dbg_hit_rst38_once {
            ...
            if pc_now == 0x0038 {
                ...
                // Debug print commented out to avoid noise
                self.dbg_hit_rst38_once = true;
            }
        }
    }

    // Apply EI delayed enable mechanism
    fn apply_ei_delay(&mut self) {
        self.ime_just_enabled = false;
        if self.ime_enable_delay > 0 {
            ...
            if self.ime_enable_delay == 0 {
                ...
                {
                    println!(
                        "[IME] delay expired -> ime=true at pc=0x{:04X}",
                        self.registers.get_pc()
                    );
                }
                self.ime = true;
                self.ime_just_enabled = true; // Mark that IME just enabled
            }
        }
    }

    // Check for pending interrupts and service if conditions met (before instruction execution)
    fn check_and_service_interrupts_before(&mut self) -> u64 {
        if self.ime_enable_delay == 0 && !self.ime_just_enabled && self.ime {
            if let Some((bit, vec)) = self.pending_interrupt() {
                #[cfg(feature = "dbg_int")]
                {
                    println!(
                        "[INT CHECK] pending int found bit={} at pc=0x{:04X}",
                        bit,
                        self.registers.get_pc()
                    );
                }
                let pc_before = self.registers.get_pc();
                let taken_int = self.service_interrupt(bit, vec, pc_before);
                self.cycles = self.cycles.wrapping_add(taken_int as u64);
                // Tick MMU timers & peripherals during interrupt service cycles
                self.memory.step(taken_int);
                // Wake up from HALT if servicing interrupt
                self.halted = false;
                return taken_int;
            }
        }
    ...
    // Check for pending interrupts and service if conditions met (after instruction execution)
    fn check_and_service_interrupts_after(&mut self) -> u64 {
        if self.ime_enable_delay == 0 && self.ime_just_enabled {
            #[cfg(feature = "dbg_int")]
            {
                println!("[INT CHECK SKIP] ime_just_enabled=true, skipping interrupt service");
            }
        }
        0
    }

    // Handle HALT state: burn 4 cycles, wake if interrupt pending
    fn handle_halt(&mut self) -> u64 {
        if let Some((_bit, _vec)) = self.pending_interrupt() {
            // Wake the CPU; interrupt will be serviced in execute_next if IME is true
            self.halted = false;
        }
        // Always burn 4 cycles
        self.cycles = self.cycles.wrapping_add(4);
        self.memory.step(4);
        4
    }

    // Fetch and decode the next opcode, handling CB prefix
    fn fetch_and_decode(&mut self) -> (u8, bool) {
        // Fetch first byte
    ...
    // Execute a single instruction and return the number of cycles taken
    fn execute_instruction(&mut self) -> u64 {
        ...
        let (opcode, cb_prefix) = self.fetch_and_decode();

        // TRACE: when running the CPU test ROM, print opcodes around test zone to aid debugging
        // (prints a small amount to avoid noise)
        let pc_for_trace = self.registers.get_pc().wrapping_sub(1);
        if pc_for_trace >= 0x0300 && pc_for_trace <= 0x0DFF {
            // debug print CPU instruction & registers snapshot
            println!("[CPU TRACE] PC=0x{:04X} OP=0x{:02X} A={:02X} B={:02X} C={:02X} D={:02X} E={:02X} F={:02X} H={:02X} L={:02X} SP={:04X}",
                     pc_for_trace, opcode, self.registers.get_a(), self.registers.get_b(), self.registers.get_c(), self.registers.get_d(), self.registers.get_e(), self.registers.get_f(), self.registers.get_h(), self.registers.get_l(), self.registers.get_sp());
        }

        // Decode
        if let Some(instr) = CPU::decode(opcode, cb_prefix) {
            // Store current registers/opcode snapshot into the debug module so bus IO (eg serial write)
            // can read the CPU state when it prints or performs IO.
            let dbg = crate::GB::debug::DebugRegs {
                pc: pc_for_trace,
                a: self.registers.get_a(),
                f: self.registers.get_f(),
            ...
            crate::GB::debug::store_state(dbg);
            // Store this executed opcode into the ring buffer so serial prints can show the
            // recent sequence of instructions at the time of interest.
            crate::GB::debug::store_op(pc_for_trace, opcode);
...
mod tests {
    // fn test_subtest_inc_b_ld_a_b_print_a_fail() {
    //     struct CPU {
    //         a: u8,
    //         b: u8,
    //     }
    ...
    //     }
    //     fn fail() {
    //         panic!("Test failed (as expected in this subtest)");
    //     }
    //     let mut cpu = CPU { a: 0, b: 0x12 };
    //     // inc b
    ...
    #[test]
    fn test_ppu_dmg_acid2_renders_frame() {
        use std::fs;
        use std::path::Path;
        // Load ROM from workspace root
        let rom_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("dmg-acid2.gb");
        if !rom_path.exists() {
            // Skip test if ROM not present in workspace
            eprintln!(
                "Skipping dmg-acid2 PPU test: ROM not found: {}",
                rom_path.display()
            );
            return;
        }
        ...
        let mut cpu = CPU::new();
        cpu.memory.load_rom(data);
        // Minimal post-BIOS initialization (match main.rs init_post_bios)
        ...
        cpu.registers.set_pc(0x0100);
        cpu.memory.write(0xFF05, 0x00);
        cpu.memory.write(0xFF06, 0x00);
        cpu.memory.write(0xFF07, 0x00);
        cpu.memory.write(0xFF26, 0xF1);
        cpu.memory.write(0xFF40, 0x91);
        cpu.memory.write(0xFF42, 0x00);
        cpu.memory.write(0xFF43, 0x00);
        cpu.memory.write(0xFF45, 0x00);
        cpu.memory.write(0xFF47, 0xFC);
        cpu.memory.write(0xFF48, 0xFF);
        cpu.memory.write(0xFF49, 0xFF);
        cpu.memory.write(0xFF4A, 0x00);
        cpu.memory.write(0xFF4B, 0x00);
        cpu.memory.write(0xFFFF, 0x00);
        // Fill VRAM similar to main init to ensure something is visible if ROM expects default tiles
        for addr in 0x8000..0x9000u16 {
            let shade = ((addr >> 5) & 0x03) as u8;
            cpu.memory.write(addr, shade);
        }

        // Run until VBlank IF is set or until cycle limit
        let mut cycles = 0usize;
        let max_cycles = 200_000usize;
        while cycles < max_cycles {
            let _ = cpu.execute_next();
            cycles = cycles.wrapping_add(1);
            if (cpu.memory.read(0xFF0F) & 0x01) != 0 {
                break;
            }
        }
        assert!(
            cycles < max_cycles,
            "did not reach VBlank within cycle limit"
        );
        // Verify framebuffer has non-zero pixels
        let fb = cpu.memory.framebuffer();
        let nonzero = fb.iter().filter(|&&b| b != 0).count();
        assert!(nonzero > 100, "framebuffer seems empty or not rendered");
    }
    ...
    #[test]
    fn test_interrupt_serviced_while_dma_active() {
        let mut cpu = CPU::new();
        // Prepare DMA source at 0x8000
        for i in 0..160u16 {
            cpu.memory.write(0x8000 + i, (i & 0xFF) as u8);
        }
        // Start OAM DMA from 0x8000
        cpu.memory.write(0xFF46, 0x80);
        // Arm VBlank interrupt and enable IME
        cpu.ime = true;
        cpu.memory.write(0xFFFF, 0x01); // IE
        cpu.memory.write(0xFF0F, 0x01); // IF

        // Executing should service the interrupt immediately (20 cycles)
        let taken = cpu.execute_next();
        assert_eq!(taken, 20);
        // IF bit should have been cleared by the interrupt service
        assert_eq!(cpu.memory.read(0xFF0F) & 0x01, 0);
        // DMA should still be active since only 20 of 640 cycles elapsed
        assert!(cpu.memory.is_dma_active());
    }

    #[test]
    fn test_dma_progresses_during_interrupt_service() {
        let mut cpu = CPU::new();
        // Prepare DMA source at 0x8000 with non-zero second byte so we can observe progress
        for i in 0..160u16 {
            cpu.memory.write(0x8000 + i, (i & 0xFF) as u8);
        }
        cpu.memory.write(0xFF46, 0x80);
        ...
        // Check byte at FE01 (should be 0 before DMA starts copying byte 1)
        let before = cpu.memory.read(0xFE01);
        let taken = cpu.execute_next();
        assert_eq!(taken, 20);
        let after = cpu.memory.read(0xFE01);
        // DMA should have progressed during the interrupt service, so FE01 should now be copied
        assert_ne!(before, after);
        // Ensure at least some OAM bytes were copied
        let mut copied = 0;
        for i in 0..160u16 {
            if cpu.memory.read(0xFE00 + i) != 0 {
                copied += 1;
...
mod boundary_tests {
    ...
    #[test]
    fn test_large_step_div_updates() {
        let mut cpu = CPU::new();
        // Reset DIV
        cpu.memory.write(0xFF04, 0xAA);
        assert_eq!(cpu.memory.read(0xFF04), 0);

        // Step a large number of cycles (1_000_000)
        cpu.memory.step(1_000_000);
        // DIV increments every 256 cycles; check wrapped value
        let expected = ((1_000_000u64 / 256u64) % 256u64) as u8;
        assert_eq!(cpu.memory.read(0xFF04), expected);
    }

    #[test]
    fn test_dma_blocks_access_and_unblocks() {
        let mut cpu = CPU::new();
        // Prepare source at 0x8000 for DMA
        for i in 0..160u16 {
            cpu.memory.write(0x8000 + i, (i & 0xFF) as u8);
        }
        // Start DMA to OAM
        ...
        // Attempt to write to a non-allowed address during DMA (0x8000).
        // The CPU wrapper should stall (step in 4-cycle chunks) until DMA completes,
        // then perform the write. instr_step_cycles should increase due to the stall.
        let before_instr_steps = cpu.instr_step_cycles;
        cpu.write8(0x8000, 0x55);
        assert!(!cpu.memory.is_dma_active());
        assert_eq!(cpu.memory.read(0x8000), 0x55);
        assert!(cpu.instr_step_cycles > before_instr_steps);
    }
}
